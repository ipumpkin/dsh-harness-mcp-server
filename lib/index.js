...
function taskProgressOf(ctx, item) {
    const info = { status: item.status };
    if (item.status === 'queued')
        return info;
    const session = liveAgentFor(ctx, item.sessionId)?.session;
    if (!session?.log)
        return info;
...
function appendPromptNotice(session, text, summary) {
    try {
        const s = session;
        s.append?.('user/message', createUserMessage({
            content: [{ type: 'text', text }],
            source: { kind: 'plugin', plugin: 'harness-mcp-server', form: 'notice', summary },
        }), { surfaceOp: 'append' });
    }
    catch (e) {
        console.warn('[harness-mcp-server] prompt notice append failed:', e?.message ?? e);
    }
}
...
function registerTools(mcp, ctx) {
    ...
    mcp.tool('agent_run', '同步执行任务(改代码/分析/跑命令), 返回结构化结果。可传 sessionId 续接已有会话(长任务分多轮投喂); 可传 newSession:true 强制全新会话(不复用池); 可传 model/provider 按次选模型; 传 timeoutMs(毫秒)超时后自动转为异步任务(返回 taskId, 用 task_result/task_cancel/task_wait 跟进)。', {
    ...
    mcp.tool('task_inbox', 'Hermes 把结构化任务(任务+记忆上下文)推入 Harness 队列, 异步执行, 返回 taskId。记忆喂编码的入口。可传 newSession:true 强制全新会话。', {
    ...
    mcp.tool('task_result', '取回 task_inbox 提交任务的结构化结果(changes/verification/leftovers); 未完成时返回 progress 供汇报进度。', { taskId: z.string().describe('task_inbox 返回的 taskId') }, async ({ taskId }) => {
    ...
    mcp.tool('task_wait', '阻塞等待一个任务完成/失败后返回其结果(服务端等待, 一次往返替代多次轮询); 超过 timeoutMs 返回当前状态与 progress(正在执行的步骤/工具调用)。客户端 HTTP 超时应大于 timeoutMs。', {
...
export async function apply(ctx, config = {}) {
    ...
    const onApprovalRequest = (req, next) => {
        ...
        return new Promise((resolve) => {
            ...
            appendPromptNotice(req.agent.session, `⏳ 该审批（${req.toolName}${req.reason !== undefined ? `：${req.reason}` : ''}）已由 MCP 侧接管处理中，请在 Hermes/客户端响应（prompt ${promptId}）`, `MCP 接管审批: ${req.toolName}`);
        });
    };
        ...
        try {
            userQuestions.registerProvider({
                ask: (request) => {
                    ...
                    const session = r.agent !== undefined ? r.agent.session : undefined;
                        ...
                        pendingQuestions.set(promptId, {
                            ...
                            resolve: (answer) => settle(() => {
                                ...
                                if (session !== undefined)
                                    appendPromptNotice(session, `✅ 提问 ${promptId} 已由 MCP 侧回答`, 'MCP 回答提问');
                                resolve(answer);
                            }),
                            reject: (e) => settle(() => {
                                if (session !== undefined)
                                    appendPromptNotice(session, `❌ 提问 ${promptId} 已取消/失败: ${e?.message ?? String(e)}`, 'MCP 取消提问');
                                reject(e);
                            }),
                        });
