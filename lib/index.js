...
async function executeTask(ctx, task, context, cwd, resumeSessionId, title, opts) {
    ...
    return withLock(lockKey, async () => {
        ...
        const { sessionId, handle, disposeAfter } = await getAgent(ctx, workdir, resumeSessionId, title, opts?.fresh || (modeRequested && resumeSessionId === undefined), { provider: opts?.provider, model: opts?.model }, opts?.mode);
