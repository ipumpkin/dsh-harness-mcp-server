...
const SANDBOX_MODE_DESCRIPTIONS = {
...
const APPROVAL_POLICY_DESCRIPTIONS = {
 ...
 */
function deriveSessionTitle(text, maxChars = 60) {
...
function renameSessionSafe(ctx, session, title) {
    try {
        const st = ctx.get('sessionTitle');
        if (!st?.rename)
            return undefined;
...
async function getAgent(ctx, cwd, sessionId, title, fresh, modelOpts, modeOpts) {
    ...
    if (modeRequested && sessionId === undefined)
        fresh = true; // 指定模式 = 强制全新会话(池会话无法安全套用新模式)
...
async function createPoolAgent(ctx, cwd, title, agentOptions, modeOpts) {
    ...
    try {
        const sess = handle.agent.session;
        if (modeOpts?.sandbox !== undefined && sess.append)
            sess.append('sandbox/mode', { mode: modeOpts.sandbox });
        if (modeOpts?.approval !== undefined && sess.append)
            sess.append('approval/policy', { policy: modeOpts.approval });
    }
    catch (e) {
...
const mcpBusySessionIds = new Set();
...
const sessionPresetApplied = new Map();
/** 从会话事件流折出生效沙箱模式(最后一条 sandbox/mode 事件; 无覆盖回退部署默认) */
function effectiveSandboxModeOf(session, fallback) {
    const events = session.events ?? [];
    for (let i = events.length - 1; i >= 0; i--) {
        const e = events[i];
        if (e?.type === 'sandbox/mode' && e.data?.mode)
            return e.data.mode;
    }
    return fallback;
}
/** 从会话事件流折出生效审批策略(最后一条 approval/policy 事件; 无覆盖回退部署默认) */
function effectiveApprovalPolicyOf(session, fallback) {
    const events = session.events ?? [];
    for (let i = events.length - 1; i >= 0; i--) {
        const e = events[i];
        if (e?.type === 'approval/policy' && e.data?.policy)
            return e.data.policy;
    }
    return fallback;
}
/** 会话生效的 preset: 本插件创建/接管时登记优先, 其次 session header 的 agentPreset(创建事实), 否则插件配置默认 */
function effectivePresetOf(sessionId, header) {
...
function sessionModeOf(ctx, sessionId, session, header) {
...
async function resolveModeRequest(ctx, input) {
    const out = {};
    ...
    if (input.mode !== undefined) {
        ...
        if (!matched && SANDBOX_MODES.includes(m)) {
            out.sandbox = m;
            matched = true;
        }
        if (!matched && APPROVAL_POLICIES.includes(m)) {
            out.approval = m;
            matched = true;
        }
...
async function listPresetIds(ctx) {
    const agentPresets = ctx.get('agentPresets');
    try {
 ...
 * 维持既有入队 + flush 路径。
 */
function notifyPromptIntercepted(agent, text, summary) {
    ...
    if (inbox?.append) {
        try {
            ...
            console.warn('[harness-mcp-server] prompt steering append failed, falling back to queued notice:', e?.message ?? String(e));
        }
    }
...
async function executeTask(ctx, task, context, cwd, resumeSessionId, title, opts) {
    ...
    return withLock(lockKey, async () => {
        ...
        const { sessionId, handle, disposeAfter } = await getAgent(ctx, workdir, resumeSessionId, effectiveTitle, opts?.fresh || (modeRequested && resumeSessionId === undefined), { provider: opts?.provider, model: opts?.model }, opts?.mode);
        ...
        try {
            ...
            const eff = sessionModeOf(ctx, sidStr, handle.agent.session, handle.agent.session.header);
            result.mode = {
                preset: eff.preset,
                sandbox: eff.sandbox,
...
function registerTools(mcp, ctx) {
    ...
    mcp.tool('mode_list', '列出可用会话模式: agent preset(standard/code/cordis/minimal 等, 来自 dsh agent-presets) + 沙箱访问模式(read-only/workspace-write/danger-full-access) + 审批策略(ask/never) + 权限预设(捆绑沙箱+审批, 如 workspace-write = workspace-write + ask)。modes 汇总给出可传给 agent_run/task_inbox 的 mode= 规范 id; 传 only 只列某一类。', {
        ...
        if (agentPresets?.list) {
            try {
                ...
                presets.push({ error: `agentPresets.list failed: ${e?.message ?? String(e)}` });
            }
        }
        else {
        ...
        const sandboxModes = SANDBOX_MODES.map((m) => ({
            id: m,
        ...
        // 3) 审批策略(固定词汇 + 部署默认标注)
        const approvalPolicies = APPROVAL_POLICIES.map((p) => ({
            id: p,
        ...
        const body = { total: 4, categories };
        if (only === undefined || only === 'preset')
            body.presets = presets;
        if (only === undefined || only === 'sandbox')
            body.sandboxModes = sandboxModes;
        if (only === undefined || only === 'approval')
            body.approvalPolicies = approvalPolicies;
        if (only === undefined || only === 'permission')
            body.permissionPresets = permissionPresetsList;
        if (only === undefined) {
            body.modes = modes;
            body.deployment = {
    ...
    mcp.tool('agent_run', '同步执行任务(改代码/分析/跑命令), 返回结构化结果, 附任务会话上下文占用 context(events/tokens/pressure/window/ratio)与生效模式 mode(preset/sandbox/approval)。可传 sessionId 续接已有会话(长任务分多轮投喂); 可传 newSession:true 强制全新会话(不复用池); 可传 model/provider 按次选模型; 可传 preset/mode/sandbox/approval 指定会话模式(创建会话时应用, 指定即强制全新会话, 避免后续再提权); 传 timeoutMs(毫秒)超时后自动转为异步任务(返回 taskId, 用 task_result/task_cancel/task_wait 跟进)。', {
    ...
    mcp.tool('task_inbox', 'Hermes 把结构化任务(任务+记忆上下文)推入 Harness 队列, 异步执行, 返回 taskId; 任务结束后的上下文占用经 task_result/task_wait/task_list 读取。可传 newSession:true 强制全新会话; 可传 preset/mode/sandbox/approval 指定会话模式(创建会话时应用, 指定即强制全新会话, 避免后续再提权)。', {
    ...
    mcp.tool('session_list', '列出可续接的会话(常驻池 / live / 持久化三层去重, 池优先), 含上下文占用 events/tokens/pressure/window/ratio(经 tokenMeter 测量 + llm 模型窗口)与生效模式 mode(preset/sandbox/approval; 池/live 行从会话日志折出, 持久化行仅 header 已知 preset); 持久化层未加载日志为 null。', {}, async () => {
        const rows = new Map();
        ...
        for (const [cwd, rec] of liveAgents) {
            ...
            rows.set(sid, {
                ...
                mode: sessionModeOf(ctx, sid, agent.session, agent.session.header),
            });
        }
...
export async function apply(ctx, config = {}) {
    ...
    if (config.authTokens?.length)
        runtimeConfig.authTokens = [...config.authTokens];
    ...
    const onApprovalRequest = (req, next) => {
        ...
        if (!mcpSessionIds.has(agentId) || !mcpBusySessionIds.has(agentId))
            return next();
        ...
        return new Promise((resolve) => {
            ...
            notifyPromptIntercepted(req.agent, `⏳ 审批已由 MCP 接管（${req.toolName}${req.reason !== undefined ? `：${req.reason}` : ''}），等待 Hermes/客户端响应（prompt ${promptId}）`, `⏳ 审批已由 MCP 接管：${req.toolName}`);
        });
    };
                        ...
                        pendingQuestions.set(promptId, {
                            ...
                            resolve: (answer) => settle(() => {
                                ...
                                if (r.agent !== undefined)
                                    queuePromptNotice(r.agent, `✅ 提问 ${promptId} 已由 MCP 侧回答`, '✅ 提问已由 MCP 回答');
                                resolve(answer);
                            }),
                            reject: (e) => settle(() => {
                                if (r.agent !== undefined)
                                    queuePromptNotice(r.agent, `❌ 提问 ${promptId} 已取消/失败: ${e?.message ?? String(e)}`, '❌ 提问已取消/失败');
                                reject(e);
                            }),
                        });
    ...
    if (typeof ctx.inject === 'function') {
        ...
        ctx.inject(['settings'], (settingsCtx) => {
            const scope = settingsCtx.settings.register(settingsNamespace(SETTINGS_NAMESPACE), HarnessMcpSettingsSchema, {
                base: {
                ...
                validate: (v) => {
                    ...
                    if (/[\r\n]/.test(v.authToken))
                        throw new Error('authToken 不能包含换行');
                },
            });
