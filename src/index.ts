...
function registerTools(mcp: McpServer, ctx: Context): void {
  ...
  mcp.tool(
    'agent_run',
    '同步执行任务(改代码/分析/跑命令), 返回结构化结果, 附任务会话上下文占用 context(events/tokens/pressure/window/ratio)。可传 sessionId 续接已有会话(长任务分多轮投喂); 可传 newSession:true 强制全新会话(不复用池); 可传 model/provider 按次选模型; 传 timeoutMs(毫秒)超时后自动转为异步任务(返回 taskId, 用 task_result/task_cancel/task_wait 跟进)。',
    {
  ...
  mcp.tool(
    'task_inbox',
    'Hermes 把结构化任务(任务+记忆上下文)推入 Harness 队列, 异步执行, 返回 taskId; 任务结束后的上下文占用经 task_result/task_wait/task_list 读取。可传 newSession:true 强制全新会话。',
    {
